...
#[test]
fn given_empty_query_or_missing_embedding_inputs_when_searching_then_it_fails_fast() {
    ...
    let missing_provider =
        search_notes_for_query(harness.root(), harness.db_path(), "query", "", "model")
            .expect_err("missing provider should fail before network operations");
    assert!(missing_provider.to_string().contains("provider"));

    let missing_model =
        search_notes_for_query(harness.root(), harness.db_path(), "query", "ollama", "")
            .expect_err("missing model should fail before network operations");
    assert!(missing_model.to_string().contains("model"));
}
