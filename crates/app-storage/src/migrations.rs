...
// depend on external migration files at runtime. Touch this file when adding a
// migration so cargo rebuilds the embedded directory snapshot.
...
pub fn appdata_db_file_name() -> &'static str {
...
pub fn resolve_appdata_db_path(app_data_dir: &Path) -> Result<PathBuf> {
    Ok(app_data_dir.join(appdata_db_file_name()))
}

pub fn run_app_migrations(app_data_dir: &Path) -> Result<PathBuf> {
    let db_path = resolve_appdata_db_path(app_data_dir)?;
...
pub fn cleanup_legacy_workspace_index_db(workspace_root: &Path) -> Result<()> {
