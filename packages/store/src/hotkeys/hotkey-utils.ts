import { getModifierKey } from "@mdit/utils/keyboard-shortcut"
...
export function hotkeyToDisplayTokens(binding: string): string[] {
	...
	return normalizedBinding
		.split("+")
		.map((token) => (token === "Mod" ? getModifierKey() : token))
}
...
function isModifier(value: string): value is (typeof MODIFIER_ORDER)[number] {
